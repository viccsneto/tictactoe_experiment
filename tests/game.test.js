...
describe("replaceSymbol", () => {
  test("replaces X with cat emoji", () => {
    expect(replaceSymbol("X", "😺", "🐶")).toBe("😺");
  });

  test("replaces O with dog emoji", () => {
    expect(replaceSymbol("O", "😺", "🐶")).toBe("🐶");
  });

  test("returns empty string for invalid input", () => {
    expect(replaceSymbol("", "😺", "🐶")).toBe("");
    expect(replaceSymbol("Z", "😺", "🐶")).toBe("");
  });
});

describe("replaceSymbols", () => {
  test("replaces the whole board with emoji symbols", () => {
    const board = ["X", "O", "", "O", "X", "", "", "", "X"];
    expect(replaceSymbols(board, "😺", "🐶")).toEqual([
      "😺",
      "🐶",
      "",
      "🐶",
  ...
  test("does not mutate the original board", () => {
    const board = ["X", "O", ""];
    const copy = replaceSymbols(board, "😺", "🐶");
